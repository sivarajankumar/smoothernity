#ifndef _shy_platform_static_array_injections_included
#define _shy_platform_static_array_injections_included

#include "src/injections/platform/math/insider/shy_insider.h"
#include "src/injections/platform/pointer/shy_pointer.h"
#include "src/profile/platform/static/array/shy_array_injections.h"
#include "src/profile/shy_profile_injections.h"
#include "src/trace/platform/static/array/shy_array_injections.h"
#include "src/trace/shy_trace_injections.h"

#include "./shy_array.h"

typedef shy_platform_static_array so_called_platform_static_array ;

#endif
