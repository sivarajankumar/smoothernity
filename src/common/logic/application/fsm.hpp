...
void shy_logic_application_fsm < mediator > :: _tick_single_fsm ( typename platform_pointer :: template pointer < _logic_application_fsm_state_type > & state )
{
    ...
    state . get ( ) . on_input ( _logic_application_fsm_state_environment ) ;
    for ( ; ; )
    {
        platform_pointer :: bind ( next_state , state . get ( ) . transition ( _logic_application_fsm_state_environment ) ) ;
        platform_pointer :: are_equal ( states_are_equal , state , next_state ) ;
        if ( platform_conditions :: whole_is_true ( states_are_equal ) )
            break ;
        else
        {
            state . get ( ) . on_exit ( _logic_application_fsm_state_environment ) ;
            state = next_state ;
            state . get ( ) . on_entry ( _logic_application_fsm_state_environment ) ;
        }
    }
}
