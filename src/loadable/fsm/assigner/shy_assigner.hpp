...
void shy_loadable_fsm_assigner :: assign ( )
{
    ...
    for ( so_called_type_loadable_fsm_content_system_container :: iterator fsm_system_i = system_container -> begin ( )
        ; fsm_system_i != system_container -> end ( )
        ; ++ fsm_system_i
        )
    {
        so_called_lib_std_string fsm_system_name = fsm_system_i -> first ;
        so_called_type_loadable_fsm_content_system & fsm_system = fsm_system_i -> second ;
        for ( so_called_type_loadable_fsm_content_machine_container :: iterator fsm_machine_i = fsm_system . machines . begin ( )
            ; fsm_machine_i != fsm_system . machines . end ( )
            ; ++ fsm_machine_i
            )
        {
            so_called_lib_std_string fsm_machine_name = fsm_machine_i -> first ;
            so_called_type_loadable_fsm_content_machine & fsm_machine = fsm_machine_i -> second ;
            if ( fsm_machine . states . find ( so_called_loadable_fsm_consts :: state_initial ) == fsm_machine . states . end ( ) )
            {
                fsm_machine . states [ so_called_loadable_fsm_consts :: state_initial ] = so_called_type_loadable_fsm_content_state ( ) ;
