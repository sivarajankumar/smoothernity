...
syntax keyword shy_statement system machine state consts on to is command if do input entry exit initial discard vars ops receive request module trace with else while args proc
syntax match shy_constant "\(\(^\| \)[\-]\? *\d\+\( *\/ *\d\+\)\?\)\|\<true\>\|\<false\>"
syntax match shy_special "[\{\}\(\)\*]\|<\-\|\->\|\<msg\|\<reply"
