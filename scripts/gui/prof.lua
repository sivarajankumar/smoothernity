...
function M.alloc(xmin, ymin, xmax, ymax, refvalue, ...)
    ...
    function self.set(...)
        ...
        barmax.set(augment(limit, ...))
        barmin.set(augment(limit, ...))
    ...
    do
        ...
        barmax = bar.alloc(xmin+b, ymin + b*1 + h*0, xmax-b, ymin + b*1 + h*1, ...)
        barcur = bar.alloc(xmin+b, ymin + b*2 + h*1, xmax-b, ymin + b*2 + h*2, ...)
        barmin = bar.alloc(xmin+b, ymin + b*3 + h*2, xmax-b, ymin + b*3 + h*3, ...)
        barback.set(1)
    end
end
