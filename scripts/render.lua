...
local function visual_alloc()
    ...
    local rclrnear = api_rop_alloc_clear(rclrdep, API_ROP_CLEAR_COLOR + API_ROP_CLEAR_DEPTH)
    local rtscale = api_rop_alloc_tscale(rclrnear, vtscale, 0)
    local rproj3dnear = api_rop_alloc_proj(rtscale, mproj3dnear)
    local rmview3d = api_rop_alloc_mview(rproj3dnear, self.mview3d)
    local rmeshnear = api_rop_alloc_draw_meshes(rmview3d, meshes.GROUP_NEAR)
