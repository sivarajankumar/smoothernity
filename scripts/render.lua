...
local function visual_alloc()
    ...
    local vclrdep2d = util.vector_const(1, 0, 0 ,0)
    local vclrdep3d = util.vector_const(1, 0, 0 ,0)
    self.vtscale = util.vector_const(1, 0, 0, 0)

    local rroot = api_rop_alloc_root()
    local rclrdep3d = api_rop_alloc_clear_depth(rroot, vclrdep3d, 0)
    local rclrcol = api_rop_alloc_clear_color(rclrdep3d, self.vclrcol)
    ...
    local rclrdep2d = api_rop_alloc_clear_depth(lods[lod.count - 1].rmesh, vclrdep2d, 0)
    local rclr2d = api_rop_alloc_clear(rclrdep2d, API_ROP_CLEAR_DEPTH)
    ...
    function self.free()
        ...
        api_vector_free(vclrdep2d)
        api_vector_free(vclrdep3d)
        api_vector_free(self.vtscale)

        api_rop_free(rroot)
        api_rop_free(rclrdep3d)
        api_rop_free(rclrcol)
        api_rop_free(rtscale)

        api_rop_free(rclrdep2d)
        api_rop_free(rclr2d)
